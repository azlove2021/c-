...
int main()
{
    ...
    int r; //进制
    string str;
    string X = "0123456789"; //16进制内   //这里有问题 a和0 Ascall码不是相差11位
    ...
    for (int k = 0; k < n; k++)
    {
        ...
        if (r <= 10)  //10进制内
            for (int i = 0; i < r; i++)
            {
                ...
                {               //j表示计数而已
                    ...
                    ans += Str[j] - '0';
                    j++;        //1234567 8进制 1 先读取 乘以了8的6次方！！  
                } //得到10进制数
                if (ans % (r - 1) == 0)
                {
                    cout << X[i] << endl;

                    pd = true; break;  //一定能找到唯一一个！！！！！！
        ...
        else  //r>10
        {
            ...
            {
                int ans = 0, j = 0;
                Str += X[ii]; //补上最后一位再转换   不能一直加下去 0-9
                while (Str.length() != j)
                {
            ...
            Str = str; //还原
            if (pd == false)     //考虑末尾补上 10-15的情况
                for (int i = 0; i < 6; i++)  //abcdef  
                {

                    int ans = 0, j = 0;
                    Str += XX[i]; //补上最后一位再转换   不能一直加下去 abcdef
                    while (Str.length() != j)
                    {
                        ans *= r;
